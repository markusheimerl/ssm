...
$$
\begin{align*}
H_t &= X_tB^T + H_{t-1}\tanh(A)^T \\
O_t &= H_t\sigma(H_t) \\
Y_t &= O_tC^T + X_tD^T
\end{align*}
$$

The state transition matrix $A$ captures temporal dependencies, with eigenvalue control achieved by routing through $\tanh(A)$ to guarantee eigenvalues bounded in $[-1, 1]$ for numerical stability. Input matrix $B$ maps current inputs to state updates, output matrix $C$ projects nonlinearly activated states to outputs, and feedthrough matrix $D$ provides direct input-output connections. The linear state evolution $H_t = X_tB^T + H_{t-1}\tanh(A)^T$ enables parallel computation via scan algorithms, while the Swish activation applied later preserves model expressiveness.
...
\frac{\partial L}{\partial H_t} &= \frac{\partial L}{\partial O_t} \odot [\sigma(H_t) + H_t\sigma(H_t)(1-\sigma(H_t))] + (\frac{\partial L}{\partial H_{t+1}})\tanh(A) \\
\frac{\partial L}{\partial A} &= \sum_t (\frac{\partial L}{\partial H_t})^T H_{t-1} \odot (1 - \tanh^2(A)) \\
